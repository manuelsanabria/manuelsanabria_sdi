...
NSString * PJD_Sanabria_Manuel$ModuleRequireFormat = @"(function(exports){"
...
@implementation PJD_Sanabria_ManuelObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_pjd_sanabria_manuel);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	// create PJD_Sanabria_Manuel global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "PJD_Sanabria_Manuel" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_pjd_sanabria_manuel = [[PJD_Sanabria_ManuelObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_pjd_sanabria_manuel];
	
	NSString *_pjd_sanabria_manuelNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _pjd_sanabria_manuelNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_pjd_sanabria_manuel valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:PJD_Sanabria_Manuel$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.pjd_sanabria_manuel.kroll" 
	...
	@throw [NSException exceptionWithName:@"org.pjd_sanabria_manuel.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}
