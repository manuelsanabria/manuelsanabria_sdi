...
//Rare exceptions to indicate a bug in the _pjd_sanabria_manuel code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_pjd_sanabria_manuel = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_pjd_sanabria_manuel]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.pjd_sanabria_manuel.module"
